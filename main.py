...
#timg kiếm
a=ctk.CTkEntry(master=root,placeholder_text="Tìm Kiếm")
a.place(x=720,y=50)
...
reg=ctk.CTkEntry(root,placeholder_text='số')
...
#entry()
ctk.CTkEntry(lb_frame,placeholder_text='Tên').place(x=180,y=10)
qw=ctk.CTkOptionMenu(lb_frame,values=['Nam','Nữ'])
qw.place(x=180,y=70)
qw.set('Chọn')
ctk.CTkEntry(lb_frame,placeholder_text='Giới Tính').place(x=180,y=130)
ctk.CTkEntry(lb_frame,placeholder_text='Địa Chỉ').place(x=180,y=190)
ctk.CTkEntry(lb_frame,placeholder_text='CONTACT').place(x=180,y=250)
#entry2
ts=ctk.StringVar()
date_muon=ctk.StringVar()
han=ctk.StringVar()
c6=ctk.CTkEntry(lb_frame,placeholder_text='Tên Sách').place(x=640,y=10)
c7=ctk.CTkEntry(lb_frame,placeholder_text='Ngày Mượn').place(x=640,y=70)
c8=ctk.CTkEntry(lb_frame,placeholder_text='Hạn').place(x=640,y=130)
...
def admin():
    ...
    if nguyen is None or not nguyen.winfo_exists() or nguyen.winfo_toplevel() != nguyen:
        ...
        Label(nguyen,text='''Hướng dẫn sử dụng:
ở ô có ngày vui lòng không thêm bất kỳ vào
có thể thêm vào ô số''').pack()
        nguyen.grab_set()
        nguyen.mainloop()
    else:
...
ctk.CTkButton(root,text='Lưu Người Mượn',command=lambda:messagebox.showinfo('Thông Tin','Dữ Liệu Đang Được Cập Nhật')).place(x=1000,y=340)
